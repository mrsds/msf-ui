/* This is copied from https://github.com/datavisyn/chartjs-plugin-error-bars.git, with a minor patch
that was just too impossible to do any other way. (Allows tooltips to appear on top of error bars). */
...
const ErrorBarsPlugin = {
    ...
    afterDatasetsDraw(chart, easingValue, options) {
        ...
        const errorBarWidths = (Array.isArray(options.width) ? options.width : [options.width]).map(
            w => this._computeWidth(chart, horizontal, w)
        );
        const errorBarLineWidths = Array.isArray(options.lineWidth)
            ? options.lineWidth
            : [options.lineWidth];
        ...
        barchartCoords.forEach((dataset, i) => {
            dataset.forEach(bar => {
                ...
                errorBars.forEach((errorBar, ei) => {
                    ...
                    const plusValue = options.absoluteValues
                        ? errorBar.plus
                        : value + Math.abs(errorBar.plus);
                    const minusValue = options.absoluteValues
                        ? errorBar.minus
